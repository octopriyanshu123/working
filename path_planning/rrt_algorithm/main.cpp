...
// DFS function to explore the grid recursively
void dfs(std::vector<std::vector<sf::Color>>& grid, int gridSize, int i, int j,sf::Vector2i src_point,sf::Vector2i desr_point) {
    

int x = rand() % 10+1;
int y = rand() % 10+1;
sf::Vector2i curr_point{x,y};
                ...
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    if (row < gridSize && col < gridSize) {
                        ...
                        if (click_count == 1) {
                            ...
                            dfs(grid, gridSize, first_click.x, first_click.y,first_click,second_click);
                        }
                    }
                }
